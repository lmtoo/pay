...
public class FuncUtils {
	...
	private final static Logger LOGGER = LoggerFactory.getLogger(FuncUtils.class);
    ...
    public static String hiddenEmail(String email) {
        ...
        if (temp.length != 2) {
            ...
            String pre = temp[0];
            if (pre.length() < 4) {
                sb.append(pre.substring(0, 1)).append("***").append("@").append(temp[1]);
            } else {
                CharSequence target = pre.subSequence(pre.length() - 3, pre.length());
                CharSequence replacement = new String("***");
                pre.replace(target, replacement);
                sb.append(pre.replace(target, replacement)).append("@").append(temp[1]);
            }
            return sb.toString();
        }
    }
    ...
    public static String getIpAddr(HttpServletRequest request) {
        return RequestUtils.getRemoteIP(request);
    }    
    /**
     * 获取本地ip
     * @return
    ...
    public static boolean isCard_ExpDate(String exp_date) {
        ...
        String month = exp_date.substring(exp_date.length() - 2, exp_date.length());
        if (Integer.parseInt(month) > 12 || Integer.parseInt(month) < 1) {
            return false;
        }
        if (Integer.parseInt(exp_date) < Integer.parseInt(DateUtil.getCurrentYearMonthStr())) {
            return false;
        }
        return true;
    }

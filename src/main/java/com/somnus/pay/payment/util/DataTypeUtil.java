...
public class DataTypeUtil {
	...
	private final static Logger LOGGER = LoggerFactory.getLogger(DataTypeUtil.class);
    ...
    public static boolean numberValidator(String targetStr) {
        ...
        for (int i = 0; i < strArray.length - 1; i++) {
            if (i == 0) {
                isBig = (strArray[i] - strArray[i + 1] == 1 || strArray[i] - strArray[i + 1] == 0) ? true
                    : false;
            }
            if (isBig)
                con = strArray[i] - strArray[i + 1] == 1 || strArray[i] - strArray[i + 1] == 0;
            else
